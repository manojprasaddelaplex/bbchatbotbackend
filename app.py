...
from functions import insertQueryLog, generateSqlQuery, readSqlDatabse, saveFeedback, findSqlQueryFromDB, extractSqlQueryFromResponse, manage_conversation_length, find_best_matching_user_questions,find_best_matching_user_question_with_sql
...
conversation_history = [
    {
        "role": "system",
        "content": '''
        1.You are Sonar Chatbot, an expert at converting natural language into SQL queries for SQL Server.
            
        2.**End all SQL queries with a semicolon. You are strictly prohibited from performing data modification tasks; only fetch data.**
        3.**For similar questions provided:
            ...
            Prioritize accuracy.**
        4.If the similar question isn't relevant, use your expertise to generate a suitable query.
        5.try not to use CAST in sql queries.
        5.**For non-SQL-related questions, keep your responses brief and relevant to your purpose.**
        '''
    }
    ]
...
def query_db():
    ...
    if best_match:
        conversation_history.append({"role": "system", "content": f"Similar question: {best_match['UserQuestion']}"})
        conversation_history.append({"role": "system", "content": f"SQL for similar question: {best_match['SqlQuery']}"})
    ...
    try:
        ...
        if((len(headers) or len(rows)) == 0):
            results = {"text":"I found 0 records matching your search. Please try asking different question or adjust your search criteria."}
